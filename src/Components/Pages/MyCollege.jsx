...
// Static data outside of the component
...
const MyCollege = () => {
  ...
  const handleReviewSubmit = (collegeId) => {
    if (review && rating) {
      const newReview = { review, rating, collegeId };
      setReviews((prevReviews) => [...prevReviews, newReview]); // Functional update
      setReview('');
      setRating(0);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      {staticColleges.length === 0 ? (
        <p className="text-center text-gray-500">You have not booked any colleges yet.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {staticColleges.map((college) => (
            <div key={college.id} className="border border-gray-300 rounded-lg p-4 shadow-md hover:shadow-lg transition">
              ...
              <p className="text-sm text-gray-600 mb-4">Research Count: {college.researchCount}</p>
              ...
              <div className="mt-4">
                ...
                <button
                  onClick={() => handleReviewSubmit(college.id)}
